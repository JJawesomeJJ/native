...
import com.example.administrator.native_app.view.StatusBarUtil;
import com.example.administrator.native_app.view.SystemBarTintManager;
...
public abstract class BaseActivity extends AppCompatActivity {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        mContext = this;
        ActivityCollector.getInstance().setRun_at(this);
        handler();
    ...
    public void handler(){
        mhander= new Handler()
        {
            ...
            public void handleMessage(Message msg) {
                switch (msg.what){
                    case 1:
                        HashMap params=(HashMap)msg.obj;
                        callback callback=(callback) params.get("callback");
                        switch (params.get("method")+""){
                            case "fail":
                                callback.fail(params.get("data")+"",params.get("url")+"");
                                break;
                            case "suceess":
                                callback.seccess((byte[]) params.get("data"),params.get("url")+"");
                                break;
                            default:
                                break;
                        }
    ...
    public void OpenNew(String url, Class<Activity> activity_name){
        Intent intent =new Intent(getBaseContext(),activity_name);
